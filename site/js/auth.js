...
// ============================================================
// AUTH — Firebase Authentication + Firestore Logging & Routing
// ============================================================

if (typeof ADMIN_EMAILS === "undefined") {
  var ADMIN_EMAILS = ["busineswithpathak@gmail.com", "pathakanubhav74@gmail.com"];
}
...
// Logs user sign-in to Firestore
async function recordLogin(user) {
  const ref = db.collection("users").doc(user.uid);
  const snap = await ref.get();
  const base = {
...
// Keep UI in sync with auth state & wipe lingering event listeners
auth.onAuthStateChanged((user) => {
  currentUser = user;
  const oldBtn = document.getElementById("authBtn");
  if (!oldBtn) return;

  // Replaces button element to wipe out all previous listeners attached elsewhere
